...
from Script.BackModels import MLP, BatchNormTrans, MySparseEmbedding
from Script.BackModels import PointerNetWork, DIN, DIEN, FMLayer, DeepFM, PNN


class BaseModel(nn.Module):
    def __init__(self, input_size, embed_size, hidden_size, sparse_embedding=False):
        ...
        self.sparse = True
        if sparse_embedding:
            self.embedding = MySparseEmbedding(input_size, embed_size)
        else:
            self.embedding = nn.Embedding(input_size, embed_size)


class PointBasedModel(BaseModel):
    def __init__(self, input_size, embed_size, predict_hidden_sizes, output_size=1,
                 dropout=0.5, model_name='PNN', sparse_embedding=False):
        super().__init__(input_size, embed_size, 0, sparse_embedding)
        predict_hidden_sizes.append(output_size)
        self.mlp = MLP(embed_size, predict_hidden_sizes, dropout=dropout, norm_layer=BatchNormTrans)
        self.requires_mlp = True
        if sparse_embedding:
            w1 = MySparseEmbedding(input_size, 1)
        else:
            w1 = nn.Embedding(input_size, 1)
        if model_name == 'FM':
            self.model = FMLayer(0, 0, w1, self.embedding, point_wise=False)
        elif model_name == 'DeepFM':
            self.model = DeepFM(w1, self.embedding, self.mlp)
            self.requires_mlp = False
        elif model_name == 'PNN':
            self.model = PNN(w1, self.embedding, embed_size)
...
class SeqBasedModel(BaseModel):
    def __init__(self, input_size, embed_size, hidden_size, predict_hidden_sizes, output_size=1, dropout=0.5,
                 model_name='LSTM', sparse_embedding=False):
        super().__init__(input_size, embed_size, hidden_size, sparse_embedding)
        predict_hidden_sizes.append(output_size)
        self.mlp = MLP(hidden_size, predict_hidden_sizes, norm_layer=BatchNormTrans, dropout=dropout)
        ...
        if model_name == 'LSTM':
            ...
            self.dec = DIN(embed_size, hidden_size, dropout)
    ...
    def forward_1(self, history, targets):  # For All Sequence Models
        ...
        target_states = self.mlp(target_states).squeeze(-1).sigmoid()  # (B, L)
    ...
    def forward_2(self, history, targets):  # For PointerNetwork Or GeneralRNN
        ...
        if self.requires_mlp:
            ...
            target_states = self.mlp(target_states).squeeze(-1).sigmoid()
        return target_states
    ...
    def forward_3(self, history, targets):  # For DIN and DIEN
        ...
        for i in range(L):
            ...
            if isinstance(target_state, tuple):
                target_state, _ = target_state
            target_states.append(target_state.squeeze(-1))
        target_states = torch.stack(target_states, dim=1)  # (B, L, H)
        target_states = self.mlp(target_states).squeeze(-1)  # (B, L)
        target_states = torch.sigmoid(target_states)
        return target_states

    def forward(self, x):
        # X : (B, 2*L, fields)
        x_e = self.embedding(x).sum(-2)
        history, targets = torch.split(x_e, dim=1, split_size_or_sections=x_e.shape[1] // 2)  # Both (B, L, E)
...
#             _input = torch_fn.sigmoid(_input)
#             _forget = torch_fn.sigmoid(_forget)
#             _cell = torch_fn.sigmoid(_cell)
#             _out = torch_fn.sigmoid(_out)
#             c_i = (_forget * c) + (_input * _cell)
#             h_i = _out * torch_fn.tanh(c_i)
#
#             hidden_i, output = self.attention(h_i, context, torch.eq(mask, 0))
#             hidden_i = torch_fn.tanh(self.hidden2out(torch.cat((hidden_i, h_i), 1)))
#
...
if __name__ == '__main__':
    ...
    x_ = torch.randint(0, 16, size=(7, 10, 3))
    y_ = model(x_)
    print(y_)
