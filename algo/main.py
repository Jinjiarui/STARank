...
from Script.SimpleDataLoader import SimpleDataset2, collate_fn_point, collate_fn_seq
...
def main(args: dict):
    ...
    delete_time = args['dataset'] in ('tmall', 'alipay', 'taobao')
    listwise_label = seq and args['dataset'] in ('MQ2007',)
    # Create Dataset
    ...
    dataset = SimpleDataset2(user_item, mode='train', click_model=click_model, ratio=args['ratio'], seq_len=10,
                             delete_time=delete_time, listwise_label=listwise_label)
    collate_fn = collate_fn_seq if seq else collate_fn_point
    dataloader = data.DataLoader(dataset, batch_size=args['batch_size'],
                                 shuffle=True, num_workers=6, collate_fn=collate_fn, pin_memory=True)
    ...
    criterion = (lambda pred, label: nll_loss(pred.log(), label)) if seq else BCELoss()
    best_loss = 1e9
    scheduler = ReduceLROnPlateau(optimizer, 'min', factor=0.9, patience=10000, verbose=True, min_lr=args['min_lr'])
    ...
    for epoch in range(args['num_epochs']):
        ...
        for i, (x, y) in enumerate(tqdm(dataloader)):
            ...
            print('Epoch:{}, batch:{}, avg_time:{:.4f} loss:{:.4f} ndcg5:{:.4f} ndcg10:{:.4f} map5:{:.4f} map10:{:.4f}'
        ...
        print("Validate loss:{:.4f} ndcg5:{:.4f} ndcg10:{:.4f} map5:{:.4f} map10:{:.4f}"
    ...
    eval_list = [torch.cat(_) for _ in eval_list]
    print(eval_list)
    print(torch.mean(eval_list[1], 0))
    loss, ndcgs, maps, acc = eva.evaluate(eval_list[1], eval_list[0], True, True)
    print("Test loss:{:.4f} ndcg5:{:.4f} ndcg10:{:.4f} map5:{:.4f} map10:{:.4f}"
          .format(loss, ndcgs[0], ndcgs[1], maps[0], maps[1]))
    print("Detail Acc:", acc)
